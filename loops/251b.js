...
const palette = ["#e12a46", "#f6eb44", "#6feb69"];
...
for (let point of points) {
  ...
  lights.push({
    ...
    mesh,
    color,
    offset: lights.length / points.length,
    frequency: Math.floor(Maf.randomInRange(5, 10)),
  });
}
scene.add(group);
...
function draw(startTime) {
  ...
  for (let light of lights) {
    const c = light.color;
    c.getHSL(hsl);
    const f = easings.InOutQuint(.5 + .5 * Math.cos((Maf.mod(t + light.offset, 1)) * Maf.TAU * light.frequency));
    hsl.l = hsl.l * f;
    light.mesh.material.color.setHSL(hsl.h, hsl.s, .5 * hsl.l);
