...
const mTrans = new THREE.Matrix4().makeTranslation(0, .05, 0);
geometry.applyMatrix(mTrans);
...
scene.fog = new THREE.FogExp2(palette.range[2], 0.065);
...
function draw(startTime) {
  ...
  for (let ptr = 0; ptr < OBJECTS; ptr++) {
    ...
    q.setFromAxisAngle(axis, r + Maf.PI / 2, Maf.PI);
    ...
    const c = gradient.getAt(.45 + .25 * Maf.parabola(Maf.mod(-t + .05 * d, 1), 1));
