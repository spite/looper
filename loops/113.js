...
float pattern( vec2 uv ){
  ...
  if (xeven == yeven) {
    ...
    //d = smoothstep(.25,.75,clamp(d,0.,1.));
    return d;
  }
}
...
`);
  ...
  float hr = 1.2*pattern(uv + vec2(-e,-e));
  float hg = 1.2*pattern(uv);
  float hb = 1.2*pattern(uv + vec2(e,e));
  float stripOffset = 2.*(hg - pattern(uv+vec2(0.,.005)));
  float modifiedRoughness = smoothstep(.1,.9,clamp(1.2*hg,0.,1.));
  diffuseColor.rgb = vec3(modifiedRoughness);// * smoothstep(vec3(.1), vec3(.9), vec3(1.-hr, 1.-hg, 1.-hb));`);
...
function draw(startTime) {
  ...
  if (mesh.material.uniforms) {
    mesh.material.uniforms.time.value = 2*t;//easings.InOutQuad(t);
  }
  mesh.rotation.y = t * Maf.TAU;
  mesh.rotation.x = -.125 * Maf.PI;
