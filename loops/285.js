...
void main(){
  if(mod(offset+5.*(time+.5*vUv.x+0.*vUv.y),1.)<.99-.25*parabola(time,1.)) discard;
  gl_FragColor = vec4(glow*.5*color*vDepth,1.);
}
`;
...
const geo = new THREE.TorusKnotBufferGeometry(1, .33, 200, 36, 2, 3);
