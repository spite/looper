...
import { ArneodoAttractor } from '../modules/arneodo-attractor.js';

const painted = Painted(renderer, { minLevel: -.1, maxLevel: .9, lightenPass: 0 });

palette.range = ["#4A9665", "#F5F9F7", "#327152", "#A4C3AE", "#AF8150"]
...
renderer.setClearColor(0xfba444, 1);
...
const attractor = new ArneodoAttractor();
//attractor.randomize();
...
for (let j = 0; j < LINES; j++) {
  ...
  const l = Math.round(Maf.randomInRange(1, 0 + 20 * j / LINES));
  mesh.material.uniforms.dashArray.value.set(1, l);
  mesh.material.uniforms.repeat.value.x = Math.round(Maf.randomInRange(1, 1 + .25 * l));
