...
float p(vec2 uv) {
  ...
  for (int i = 0; i < OCTAVES; i++) {
    uv = vec2(perlin(uv.xy,dim), perlin(uv.yx,dim))/10.;
...
void main(){
  ...
  vec2 uv2 = mod(uv*vec2(3.,1.), vec2(1.));
  float n = .5 + .5 * p(mod(uv2+vec2(time),vec2(1.)));
  color *= n;
  n *= parabola(mod(uv2.x+time,1.),.25);
  n += parabola(mod(uv2.x+time,1.),50.);
  float n2 = 1.-smoothstep(.15,.85,n);
  n2 *= 1.-parabola(mod(uv2.x+time,1.),20.);
  if(n<.5) discard;
  //color *= 1.-smoothstep(.1,.9,parabola(mod(uv2.x+time,1.),5.));
  color /= 3.;
...
function draw(startTime) {
  ...
  //group.rotation.x = ~~(t * 2) * Maf.PI + easings.OutQuad((t * 2) % 1) * Maf.PI;

  post.render(scene, camera);
}

export { renderer, draw, loopDuration, canvas, camera };