...
const geo = new RoundedExtrudedPolygonGeometry(.33,.5,SIDES,1,.1,.25,5);
...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    ...
    const a = Math.atan2(dx,dy)/Maf.PI;
    const delay = a - .1*Math.sqrt(dx*dx+dy*dy);

    //o.pivot.position.z = 4 * Math.sin(t * Maf.TAU + delay);
    o.pivot.scale.setScalar(.5 + .5 * Math.cos(easings.InOutQuad(Maf.mod(2*t+delay,1))*Maf.TAU));
    o.pivot.rotation.x = easings.InOutQuad(Maf.mod(2*t+delay,1)) * Maf.PI;
  });
