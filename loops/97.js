...
const camera = getCamera(90);
const scene = new THREE.Scene();
const group = new THREE.Group();

function getMaterial() {
  const material = new THREE.MeshStandardMaterial({color: 0x5186a6, metalness: .1, roughness: .5, side: THREE.BackSide});
...
float pattern(float v, float v2) {
    return smoothstep( .45, .55, .5 + .5 * sin(-opacity * 20. * M_PI + v* 20. * M_PI + v2 * 10. * M_PI));
}
`);
...
const mesh = new THREE.Mesh(
  new THREE.TorusBufferGeometry(2.5,2,36,200),
  getMaterial()
);
...
function draw(startTime) {
  ...
  mesh.rotation.y = Maf.TAU / 16 * .5 * Math.cos(t*Maf.TAU);
  mesh.rotation.x = Maf.TAU / 16 * .5 * Math.sin(t*Maf.TAU);
