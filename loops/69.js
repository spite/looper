...
${shader.vertexShader}`;
    shader.vertexShader = shader.vertexShader.replace(
        ...
        `
  float nn1 = snoise(.5*position + vec3(t1,0.,2.*t1)) + snoise(.5*position + vec3(4.*t1,2.*t1,0.));
  float nn2 = snoise(.5*position + vec3(t2,0.,2.*t2)) + snoise(.5*position + vec3(4.*t2,2.*t2,0.));
  vNoise = mix(nn1,nn2,1.-time);
  transformed = .75  * position + 1.5  * normal * (.5 + .5 * vNoise);
  vNoise = .5 + .5 * vNoise;
  vNoise *= (.5 + .5 * snoise(.5*transformed));
  vNoise = .5 + .5 * vNoise;
  vUv = uv;
  #include <project_vertex>`);
...
${shader.fragmentShader}`;
    shader.fragmentShader = shader.fragmentShader.replace(
        ...
        `vec4 diffuseColor = vec4( diffuse, opacity );
diffuseColor.rgb = vec3(vNoise);
#define TAU 6.28318530718
float pattern = .5 + .5 * sin(vUv.y*75. + vUv.x * TAU + time * TAU);
if (pattern>.5) {
  discard;
}`
        );
