...
const painted = Painted(renderer, { minLevel: -.4, maxLevel: 1, lightenPass: 1 });

palette.range = ["#F81B05", "#AD0706", "#EE5E0F", "#F29A11", "#2317CC"];
...
renderer.setClearColor(0xF1AF5B, 1);
...
const attractor = new HadleyAttractor();
//attractor.randomize();
...
for (let j = 0; j < LINES; j++) {
  ...
  const l = Math.round(Maf.randomInRange(1, 5 + 55 * j / LINES));
  mesh.material.uniforms.dashArray.value.set(1, l);
  mesh.material.uniforms.repeat.value.x = Math.round(Maf.randomInRange(1, 1.1 * l));
  mesh.g.setGeometry(vertices);
  // mesh.rotation.y = .1 * j * Maf.TAU / LINES;
  mesh.position.x = -.1;
