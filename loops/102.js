...
const objects = [];
const r = .5;
const geo = new RoundedBoxGeometry(r,r,1.5*r,.05,5);

const CUBES = 4;
const RINGS = 24;
for (let j=0; j<RINGS; j++) {
  const material = new THREE.MeshStandardMaterial({color: 0xdedede, metalness: .1, roughness: .3});
...
function draw(startTime) {
  ...
  objects.forEach( (o,i) => {
    o.pivot.rotation.z = -t * Maf.TAU / CUBES + i*.5*Maf.TAU/objects.length;
  });
