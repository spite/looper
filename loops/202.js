...
palette.range = ["#277016", "#D23D0E", "#53483F", "#A60B10", "#D23D0E", "#277016"];
...
const geometry = new THREE.CylinderBufferGeometry(.33, 0, 2, 18);
...
scene.fog = new THREE.FogExp2(palette.range[2], .5);
...
function draw(startTime) {
  ...
  for (let ptr = 0; ptr < OBJECTS; ptr++) {
    ...
    const s = easings.InOutQuad(.6 + .4 * Math.sin(Maf.TAU * Maf.mod(ptr / OBJECTS + t, 1), 1));
    ...
    mRot.makeRotationX(easings.InOutQuad(Maf.mod(t + ptr / OBJECTS, 1)) * Maf.TAU);
    m.multiply(mRot);
    ...
    const s2 = Maf.map(0, 1, .5, 1, s);
    scaleValues[ptr * 3 + 0] = s2;
    scaleValues[ptr * 3 + 1] = s2;
    scaleValues[ptr * 3 + 2] = s2;

    const c = gradient.getAt(Maf.mod(ptr / OBJECTS + t, 1));
