...
const geo = new RoundedExtrudedPolygonGeometry(.5,2,4,1,.1,.25,5);
...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    ...
    const delay = Math.sqrt(dx*dx) + .1 *Math.abs(dx) + .1 * Math.abs(dy);
    let tt = Maf.mod(t - delay, 1);

    o.pivot.position.z = sign * Math.sin(tt * Maf.TAU);
  });
