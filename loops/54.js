...
function getMaterial(color, roughness, opacity) {
  const material = new THREE.MeshStandardMaterial({color, metalness: .1, roughness, opacity, side: THREE.DoubleSide});
...
float pattern(vec3 pos){
  ...
  float v = theta / M_TAU;
  float v2 = phi / M_PI;
  float m1 = .5+.5*sin(v*M_TAU*16. + v2 * M_TAU);
  float res = m1;
  return res;
}
`);
...
// hack because onBeforeCompile doesn't understand template literals,
// and MeshDepthMaterial doesn't have any uniforms we can hijack
...
const mesh = new THREE.Mesh(effect.generateGeometry(), getMaterial(0xff7800, .25));
mesh.castShadow = mesh.receiveShadow = true;
mesh.customDepthMaterial = getDepthMaterial();
mesh.scale.set( 5, 5, 5 );
group.add(mesh);

const mesh2 = new THREE.Mesh(effect.generateGeometry(), getMaterial(0xffffff, .5, .5));
mesh2.castShadow = mesh2.receiveShadow = true;
mesh2.customDepthMaterial = getDepthMaterial2();
...
function draw(startTime) {
  ...
  for (const blob of blobs) {
    ...
    const s2 = 1 + .75 * Math.sin(t*Maf.TAU);
