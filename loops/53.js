...
function getMaterial(color, roughness) {
  const material = new THREE.MeshStandardMaterial({color, metalness: .1, roughness, alphaTest: .5, side: THREE.DoubleSide, opacity: 1});
...
float pattern(vec3 pos){
  ...
  float v = theta / M_TAU;
  float m1 = .5+.5*sin(v*M_TAU*32. + opacity*M_TAU);
  float res = m1;
  return res;
}
`);
   ...
   shader.fragmentShader = shader.fragmentShader.replace(
      ...
      diffuseColor.a = clamp((1.-smoothstep(.49,.51,strip)),0.,1.);`);
...
const mesh = new THREE.Mesh(effect.generateGeometry(), getMaterial(0xb70000, .25));
mesh.material.opacity = 0;
mesh.castShadow = mesh.receiveShadow = true;
mesh.scale.set( 5, 5, 5 );
group.add(mesh);

const mesh2 = new THREE.Mesh(effect.generateGeometry(), getMaterial(0xffffff, .5));
mesh2.material.opacity = .5;
