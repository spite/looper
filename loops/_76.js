...
float displacement(vec3 pos) {
  float noise1 = 1.-abs(noise3d( pos + 2.*t1 ));
  float noise2 = 1.-abs(noise3d( pos + 2.*t2 ));
  float noise12 = 1.-abs(noise3d( .5*pos + t1 ));
  float noise22 = 1.-abs(noise3d( .5*pos + t2 ));
  return mix(noise1*noise12, noise2*noise22, 1. - time);
}
vec3 distort(vec3 pos) {
  float d = displacement(pos);
  return pos + normalize(pos) * smootherstep(0.,1.,d);
}
${shader.vertexShader}`;
    shader.vertexShader = shader.vertexShader.replace(
        `#include <project_vertex>`,
        `
  vec3 pos = position;//(modelMatrix * vec4(position,1.)).xyz;
  transformed = distort(pos);
  vUv = uv;

  float epsilon = .00001;
  float base = displacement(pos);
  vec3 px = pos + vec3(epsilon,0.,0.);
  float dx = displacement(px);
  vec3 py = pos + vec3(0.,epsilon,0.);
  float dy = displacement(py);
  vec3 pz = pos + vec3(0.,0.,epsilon);
  float dz = displacement(pz);
  vN = vec3(dx-base, dy-base, dz-base);
  vN = normalize(vN);
