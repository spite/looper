...
${shader.vertexShader}`;
    shader.vertexShader = shader.vertexShader.replace(
        ...
        `
  float noise1 = turbulence( .005 * normal + .005*t1 );
  float noise2 = turbulence( .005 * normal + .005*t2 );
  float b1 = noise3d( 1. * position + 2.*t1) + noise3d( .01 * position + 1.*t1);
  float b2 = noise3d( 1. * position + 2.*t2) + noise3d( .01 * position + 1.*t2);
  float displacement = mix(2.*noise1 + 4.*b1, 2.*noise2 + 4.*b2, 1. - time);
  transformed = position + normal * .1*displacement;
  vNoise = displacement;
  vUv = uv;
  #include <project_vertex>`);
