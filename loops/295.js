...
void main(){
  float speed = 5.;
  float repeat = 1.;
  gl_FragColor = vec4(vDepth*vec3(parabola(mod(repeat*vUv.x+offset+speed*time,1.),100.)/3.)*color,parabola(mod(repeat*vUv.x+offset+speed*time,1.),20.));
}
`;
...
const geo = new THREE.TorusKnotBufferGeometry(1, .25, 200, 18, 2, 5);
...
function draw(startTime) {
  ...
  mesh.rotation.x = easings.InOutCubic(t) * Maf.TAU;
  mesh2.rotation.y = easings.InOutCubic(Maf.mod(t + 1 / 3, 1)) * Maf.TAU;
  mesh3.rotation.z = easings.InOutCubic(Maf.mod(t + 2 / 3, 1)) * Maf.TAU;
