...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    ...
    const s = .01 + .05 * Maf.parabola(t,4);
    const off = 3;// + 1 * Maf.parabola(t,4);
    const dx = s * o.d * Math.cos(o.a+t*Maf.TAU);
    const dy = s * o.d * Math.sin(o.a+t*Maf.TAU);

    const px = dx + off + .1 * ( .5 + .5 * Math.sin(t * Maf.TAU));
    const py = dy + off + .1 * ( .5 + .5 * Math.sin(t * Maf.TAU));
    const offset = noise.simplex3(px, py, .1);

    o.pivot.scale.setScalar((.5 + .5 * Math.cos(easings.InOutQuad(offset)*Maf.TAU)));
    o.pivot.position.z = 2 - 4 *  offset;
  });
