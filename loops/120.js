...
for (let j=0; j<MAX; j++) {
  ...
  const mesh = new THREE.Mesh(
    new RoundedExtrudedPolygonGeometry(2*r,h,SIDES),
...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    ...
    o.position.x = id * ( -.6 + 1.2 * Maf.parabola(easings.InOutQuad(t1),1));
    o.position.y = id * ( .6 - 1.2 * Maf.parabola(easings.InOutQuad(t2),1));
    o.rotation.z = (id%2?-1:1) * t * Maf.TAU / SIDES + .1 * id;
  });
