...
const LINES = 8;
const LOOPS = LINES * 8;
const OBJECTS = LOOPS * 10;

const geometry = new RoundedBoxGeometry(1, 1, 1, .1, 4);
...
function draw(startTime) {
  ...
  for (let ptr = 0; ptr < OBJECTS; ptr++) {
    ...
    const a2 = ((ptr % LOOPS) * Maf.TAU / LOOPS) + offset * Maf.TAU + ptr * Maf.TAU / OBJECTS - t * .92;
    const r2 = .5;
    ...
    const s = (.05 + .05 * p.length() * p.length()) * (1.1 + Math.sin(ptr * Maf.TAU / OBJECTS + t * Maf.TAU));
