...
void main(){
  ...
  vec2 c = mod(vec2(2.,1.)*vUv+vec2(time,t),vec2(1.));
  vec2 ss = vec2(100.,10.);
  float n = p(mod(vUv * vec2(10.,1.) + vec2(0.,time), vec2(1.)) * vec2(1., 1.));
  n -= .5;
  n += 2.*parabola(mod(vUv.x+t,1.),1.);
  float n2 = smoothstep(.15,.85,n);
  if(n<.5) discard;
  color += clamp(1.-2.*smoothstep(.45,.55,n2-.5),0.,1.);
  color /= 4.;
...
const r = .2;
const geo = new THREE.TorusKnotBufferGeometry(.5, .15, 200, 36, 3, 2);
...
function draw(startTime) {
  ...
  group.rotation.z = 1 * t * Maf.TAU / 2;
