...
for (let j=0; j<RINGS; j++) {
  ...
  for (let k=0; k<OBJECTS; k++) {
    ...
    mesh.userData.r = Maf.randomInRange(0,5);
    mesh.userData.offset = Maf.randomInRange(0,Maf.TAU);
    pivot2.add(mesh);
    meshes.push(mesh);
  }
...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    const r = 3 + .5 * ( .5 + .5 * Math.sin(t*Maf.TAU));
    ...
    o.pivot.rotation.z = a + Math.sin(t*Maf.TAU) * Maf.TAU / 32;

    o.meshes.forEach( (mesh) => {
      mesh.position.y = mesh.userData.r * (.5 + .5*Math.sin(t*Maf.TAU));
      mesh.rotation.x = t * Maf.TAU;
      mesh.scale.setScalar(.1 + .1 * (.5 + .5 * Math.sin((3*t+id/RINGS)*Maf.TAU)));
    });
