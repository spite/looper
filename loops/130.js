...
for (let j=0; j<MAX; j++) {
  ...
  const mesh = new THREE.Mesh(
    new RoundedExtrudedPolygonGeometry(r,h,SIDES),
...
function draw(startTime) {
  ...
  objects.forEach( (o, id) => {
    ...
    o.position.x = .75 * id * ( .6 * Math.cos(t*Maf.TAU));
    o.position.y = .75 * id * ( .6 * Math.sin(t*Maf.TAU));
    o.rotation.z = t * Maf.TAU / SIDES + .1 * Math.sin(t * Maf.TAU) * id;
  });
