...
void main(){
  ...
  if(mod(offset+1.*(time+10.*vUv.x+1.*vUv.y+offset),1.)<.9) discard;
  if(sin(20.*(vUv.x+time)*TAU)>0.) discard;
  gl_FragColor = vec4(clamp(glow*.5*color*vDepth,vec3(0.),vec3(1.)), 1.);
}
`;
...
const geo = new THREE.TorusKnotBufferGeometry(1, .33, 200, 36, 3, 1);
...
group.add(mesh);
const geo2 = new THREE.TorusKnotBufferGeometry(1, .33, 200, 36, 3, 1);
