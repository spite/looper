...
palette.range = ["#F09C64", "#CA7644", "#E1B196", "#0A0D12", "#F16014", "#BC3915", "#F99819", "#5C3631", "#F09C64"];
...
const LINES = 12;
const LOOPS = LINES * 12;
const OBJECTS = LOOPS * 8;

const geometry = new RoundedBoxGeometry(1, .5, 3, .2, 4);
...
function draw(startTime) {
  ...
  for (let ptr = 0; ptr < OBJECTS; ptr++) {
    ...
    const r = .5 * (.45 + .05 * Math.cos(t * Maf.TAU));
    const a = ptr * Maf.TAU / OBJECTS;
    b.set(r * Math.cos(a), 0, r * Math.sin(a));
    const offset = ((ptr % LOOPS) % LINES) / LINES;
    const a2 = ((ptr % LOOPS) * Maf.TAU / LOOPS) + offset * Maf.TAU + ptr * Maf.TAU / OBJECTS + t * Maf.TAU / 11;
    const r2 = 1;
    p.set(r2 * Math.cos(a2), 1.75 * r2 * Math.sin(a2), 0);
    ...
    const s = (.05 + .1 * r) * Math.abs(Math.sin(a));
    scaleValues[ptr * 3 + 0] = s;
    scaleValues[ptr * 3 + 1] = s;
    scaleValues[ptr * 3 + 2] = s;

    const c = gradient.getAt(Maf.mod(a2 / Maf.TAU + .75, 1));
