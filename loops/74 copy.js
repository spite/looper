...
float distort(vec3 position) {
  float noise1 = 1.-abs(noise3d( .05*scale*position + 2.*t1 ));
  float noise2 = 1.-abs(noise3d( .05*scale*position + 2.*t2 ));
  float noise12 = 1.-abs(noise3d( .025*scale*position + 2.* t1 ));
  float noise22 = 1.-abs(noise3d( .025*scale*position + 2.*t2 ));
...
const geo = new THREE.TorusKnotBufferGeometry(2.,.2,200,36,1,3);
