...
palette.range = ["#EBFFAF", "#A9CA42", "#6DA818", "#1D2705", "#4B5F0E"];
...
const geometry = new RoundedExtrudedPolygonGeometry(.55, 2, 6, 1, .05, .05, 4);
...
function draw(startTime) {
  ...
  for (let ptr = 0; ptr < OBJECTS; ptr++) {
    ...
    const v = phase + ARMS * Math.atan2(dy, dx) / Maf.TAU;
    const offset = easings.InOutQuad(Maf.parabola(Maf.mod(.1 * d - t, 1), 10));
    const r = t * Maf.PI - v * Maf.PI;

    let s = .1 * offset + easings.InOutQuad(Maf.parabola(Maf.map(0, Maf.PI, 0, 1., Maf.mod(r, Maf.PI)), 1));
    if (d < 1) s = .1 * offset + 1;
    scaleValues[ptr * 3] = s;
    ...
    const c = gradient.getAt(Maf.mod(.25 + .25 * phase, 1));
    const hsl = c.getHSL();
    c.setHSL(hsl.h - .01 * s, hsl.s - .2, hsl.l + .25 * offset);
